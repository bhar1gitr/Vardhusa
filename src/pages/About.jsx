    ...
    <>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2">
        <motion.div
          ...
          className="bg-brand-600 text-white px-6 sm:px-10 lg:px-14 py-16 lg:py-24 flex flex-col justify-center"
        >
          <h1 className="font-display font-extrabold text-[38px] sm:text-[48px] leading-[1.05] mb-6">
            Know About Us!
          </h1>
        ...
        <motion.div
          ...
          className="px-6 sm:px-10 lg:px-14 py-16 lg:py-24 flex flex-col justify-center"
        >
          <h2 className="font-display font-extrabold text-[38px] sm:text-[48px] leading-[1.05] text-ink mb-6">
            History
          </h2>
      ...
      <Reveal y={0} className="relative overflow-hidden min-h-[420px] sm:min-h-[480px] flex items-center justify-center rounded-t-[28px] -mt-1">
        ...
        <motion.h2
          ...
          transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
          className="relative z-10 font-display font-extrabold text-white text-[32px] sm:text-[48px] text-center leading-[1.1] px-6 max-w-4xl"
        >
      ...
      <section className="bg-mist py-20 md:py-28">
        <div className="container-wrap max-w-3xl text-center flex flex-col items-center">
          <Reveal>
            <h2 className="font-display font-extrabold text-[28px] sm:text-[38px] leading-[1.15] text-ink">
              Founded as a Company Specializing in MEP
            </h2>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="text-brand-600 font-display font-bold text-[17px] mt-2">
          ...
          <Reveal delay={0.3}>
            <p className="text-ink/50 text-[15px] leading-relaxed mt-4">
          ...
          <motion.span
            ...
            className="text-brand-600 mt-10"
          >
      ...
      {/* CSR */}
      <section className="bg-mist pt-20 md:pt-28 pb-6">
        <div className="container-wrap">
          <Reveal>
            <div className="bg-white rounded-[28px] p-8 sm:p-12 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
              <div>
                <h2 className="font-display font-extrabold text-[30px] sm:text-[38px] leading-[1.15] text-ink mb-5">
                  We believe in giving back to <span className="text-brand-600">the Society</span>
                </h2>
                <p className="text-ink/50 text-[15px] leading-relaxed mb-8 max-w-lg">
...
function SplitSection({ title, highlight, text, img, imageSide = "right", bg = "bg-white", cta }) {
  const imageBlock = (
    <motion.div
      ...
      viewport={{ once: true, amount: 0.3 }}
      transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
      className="rounded-3xl overflow-hidden min-h-[320px] sm:min-h-[420px]"
    >
  ...
  const textBlock = (
    <Reveal delay={0.1}>
      <h2 className="font-display font-extrabold text-[30px] sm:text-[42px] leading-[1.1] text-ink mb-5">
        {title} <span className="text-brand-600">{highlight}</span>
      </h2>
      <p className="text-ink/50 text-[15px] leading-relaxed max-w-lg">{text}</p>
      {cta && (
        <button className="mt-8 flex items-center gap-2 bg-navy-800 hover:bg-navy-700 text-white rounded-full pl-6 pr-2 py-2.5 text-[14.5px] font-semibold transition-colors">
          {cta}
          <span className="bg-white/15 rounded-full p-2">
            <ArrowRight size={16} />
          </span>
